...
public class Main extends DefaultApplication {
    ...
    /**
     * The {@link QueryEngine2ViatraCep} mapping ensures the creation of atomic event instances upon graph pattern match
    ...
    public Main() {
        super(EventContext.STRICT_IMMEDIATE, CepFactory.getInstance().allRules());
    }

    /**
     * Setting up the application requires preparing the model (standard EMF things, see {@link #prepareModel()}), and
     * instantiating the mapping by registering the root graph pattern matching scope (e.g. a {@link ResourceSet}) to
    ...
    /**
     * The {@link QueryEngine2ViatraCep} mapping is disposed properly by invoking the
     * {@link QueryEngine2ViatraCep#dispose()} method.
     */
    @After
    ...
    /**
     * Preparing the model by instantiating the required objects and putting the model into a {@link ResourceSet}.
     */
